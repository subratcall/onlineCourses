...
function best_x = find_best_match(patch, strip)
    ...
    patch_row = patch_size(1);
    patch_col = patch_size(2);  
    
    num_pixel =  patch_row*patch_col;
    
    strip_size = size(strip);
    strip_row = strip_size(1);
    strip_col = strip_size(2);
    
    max_x = strip_col - patch_col;
    best_err = inf;
    for i=1:max_x
		temp_strip = strip(:, i:i+(patch_col-1));
		err = sumsq((patch - temp_strip)(:));
		if (err < best_err)
			best_err = err;
			best_x = i;
		endif
